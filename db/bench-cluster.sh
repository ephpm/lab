...
# Lanes S, W and P1 run on the newest published image. Lanes P2 and P3
# need per-site CLUSTERED replication (ephpm#416), which is NOT in any
# published image at the time of writing: it first appears in the v0.8.6
# tag, whose Docker images are not on Docker Hub yet. Point
# EPHPM_PERSITE_CLUSTER_IMAGE at a v0.8.6+ build to run them; otherwise
# the mode gate refuses those two lanes with a message saying so, which
# is the entire reason the gate exists (see "GATES" below).
IMG="${EPHPM_IMAGE:-docker.io/ephpm/ephpm:v0.8.5-php8.5}"
